...
class WapController extends WapBaseController {
	...
	function index() {
		...
		switch ($info ['game_type']) {
			case 1 :
				$shareData ['imgUrl'] = $info ['cover'] ? get_cover_url ( $info ['cover'] ) : SITE_URL . '/Addons/Draw/View/Public/guaguale_cover.jpg';
				break;
			case 2 :
				$shareData ['imgUrl'] = $info ['cover'] ? get_cover_url ( $info ['cover'] ) : SITE_URL . '/Addons/Draw/View/Public/dzp_cover.jpg';
				break;
			case 3 :
				$shareData ['imgUrl'] = $info ['cover'] ? get_cover_url ( $info ['cover'] ) : SITE_URL . '/Addons/Draw/View/Public/zjd_cover.jpg';
				break;
			case 4 :
				$shareData ['imgUrl'] = $info ['cover'] ? get_cover_url ( $info ['cover'] ) : SITE_URL . '/Addons/Draw/View/Public/nine_cover.jpg';
				break;
		}
		...
		if ($openid == '-1' || ! C ( 'USER_OAUTH' )) {
			...
			$has_subscribe = intval ( M ( 'apps_follow' )->where ( $map )->getField ( 'has_subscribe' ) );
		}
	...
	function get_state() {
		$id = I ( 'id' );
		$state = M ( 'lucky_follow' )->where ( ['id' => $id ] )->getField ( 'state' );
		echo $state;
	}
	// 大转盘
	function dzp() {
