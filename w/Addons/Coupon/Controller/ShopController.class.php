...
class ShopController extends ManageBaseController {
	...
	function _initialize() {
		...
		$param ['mdm'] =  I( 'mdm' );
		$res ['title'] = '优惠券';
		$res ['url'] = addons_url ( 'Coupon://Coupon/lists', $param );
		$nav [] = $res;
		
		$param ['coupon_id'] = I ( 'coupon_id' );
		$res ['title'] = '门店管理';
		$res ['url'] = addons_url ( 'Coupon://Shop/lists', $param );
		$res ['class'] = 'current';
		$nav [] = $res;
	...
	function lists() {
		...
		$normal_tips = '门店导航显示链接：<a id="copyLink" data-clipboard-text="' . addons_url ( 'Card://Wap/storeList', array (
				'wpid' => $public_info ['id'] 
